...
/**
 * Represents the emitter's own listener-error handler — the `error` option, invoked when a
 * listener throws during `emit`, with the caught error and the stringified event name.
 *
 * @remarks
...
/**
 * Represents the contract a consumer of an emitter holds: the `destroyed` reading, the
 * `on` / `once` / `off` registration trio, the synchronous `emit`, and the
 * `count` / `clear` / `destroy` set that reports on and releases listeners.
 */
export interface EmitterInterface<TMap extends EventMap> {
	...
	/**
	 * Removes a listener registered for an event by its original handler, including one
	 * registered through `once`.
	 *
	...
	/**
	 * Invokes an event's listeners synchronously, in registration order, isolating a throw.
	 * Does nothing after `destroy()`.
	 *
	 * @remarks
	 * Every listener runs, and an isolated throw routes to {@link EmitterOptions.error}
	 * rather than being rethrown. The listeners are snapshotted before the loop, so one
	 * registered during this call does not run in it.
	 *
	...
	/**
	 * Returns the live listener count, for one event or across every event.
	 *
	...
	/**
	 * Drops registered listeners, for one event or every event, leaving the emitter usable and
	 * `destroyed` unchanged.
	 *
